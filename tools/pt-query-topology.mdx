...
sidebarTitle: "pt_query_topology"
description: "pt_query_topology reads the currently loaded devices and their positions from a running Packet Tracer canvas via the HTTP bridge."
---

`pt_query_topology` sends a `queryTopology()` call to the Packet Tracer Script Engine via the HTTP bridge and returns a structured list of every device currently on the active logical canvas. Each entry includes the device name, type, model, and canvas coordinates. This is the primary way to inspect what is running in Packet Tracer before making modifications with other live tools.
