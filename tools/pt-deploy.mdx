---
title: "pt_deploy: Copy-to-Clipboard Deployment"
sidebarTitle: "pt_deploy"
description: "pt_deploy copies the PTBuilder JavaScript script to the clipboard and provides instructions for manually pasting it into Packet Tracer's Builder Code Editor."
---
...
After `pt_deploy` returns, follow these steps to build the topology:

1. Open **Cisco Packet Tracer 8.2+**
2. Go to **Extensions → Builder Code Editor** (or **Extensions → Scripting** depending on your PT version)
...
<Tip>
  After running `pt_deploy`, the exported `full_build.js` file contains both the structural PTBuilder calls and the IOS CLI configs as `/* */` comments — useful as a complete single-file reference for the topology.
</Tip>
