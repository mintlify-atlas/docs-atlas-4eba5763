---
title: "pt_validate_plan: Validate a TopologyPlan"
sidebarTitle: "pt_validate_plan"
description: "pt_validate_plan checks a TopologyPlan against 24 typed error codes, flagging device model errors, port conflicts, cable mismatches, and IP issues."
---

`pt_validate_plan` runs a structured validation pass over a `TopologyPlan` JSON and returns a typed result containing errors, warnings, and a validity flag. The validation engine checks four categories independently — device models, cable types and port conflicts, IP addressing, and DHCP — and collects all issues before returning, so you see every problem in a single call rather than fixing one at a time. Each error carries a machine-readable `error_code` from a fixed set of 24 codes, a human-readable `message`, the affected `device`, and a `suggestion` for how to fix it.
...
The validator reports errors using a fixed taxonomy of `ErrorCode` values. The table below groups them by category.
