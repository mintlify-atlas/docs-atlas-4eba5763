---
title: "Plan Network Topologies from Natural Language"
...
| `star` | 1 central router connected to N switches (hub-and-spoke variant) | `ospf` |
| `hub_spoke` | 1 hub router + N spoke routers, each spoke with its own LAN | `eigrp` |
...
The simplest call passes a natural-language description. You can optionally specify a template, routing protocol, number of routers, PCs per LAN, and more.

```json
{
  "description": "Build a network with 2 routers, 2 switches, 4 PCs, DHCP and static routing",
  "template": "multi_lan",
  "routing": "static",
  "routers": 2,
  "pcs_per_lan": 2,
...
<Accordion title="Use a template for common patterns">
  Saying "hub-and-spoke topology" will pick the `hub_spoke` template with EIGRP routing
  and 4 routers by default, which is much more efficient than describing every parameter.
</Accordion>
